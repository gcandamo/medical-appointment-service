...
export const main: SQSHandler = async (event) => {
  for (const r of event.Records) {
    ...
    await insertAppointmentToRDS({ insuredId: payload.insuredId, scheduleId: payload.scheduleId, countryISO: 'CL' });
    await publishConfirmation({ insuredId: payload.insuredId, scheduleId: payload.scheduleId, countryISO: 'CL' });
  }
};
